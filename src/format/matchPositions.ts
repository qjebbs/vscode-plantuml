...
export class MatchPositions {
    ...
    public GetUnmatchedTexts(): UnmatchedText[] {
        ...
        for (let i = 1; i < posCount - 1; i += 2) {
            ...
            if (s <= e) texts.push({ text: this._line.substring(s, e + 1), offset: s });
        }
        if (this._positions[posCount - 1] < this._line.length - 1) {
            texts.push({ text: this._line.substring(this._positions[posCount - 1] + 1, this._line.length), offset: this._positions[posCount - 1] + 1 });
        }
        return texts;
    }
}