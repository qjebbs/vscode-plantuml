...
import { baseHttpExporter } from './exporter/baseHttpExporter';
...
class URLMaker {
    ...
    private async makeDocumentURL(all: boolean) {
        ...
        if (!format) {
            format = await vscode.window.showQuickPick(baseHttpExporter.formats());
            if (!format) return;
        }
        let ds = new Diagrams();
        if (all) {
        ...
        let urls = this.makeURLs(ds.diagrams, config.server, format, bar)
        bar.dispose();
    ...
    makeURL(diagram: Diagram, server: string, format: string, bar: vscode.StatusBarItem): pURL {
        ...
        return <pURL>{ name: diagram.title, url: baseHttpExporter.makeURL(diagram, format) };
    }
}
